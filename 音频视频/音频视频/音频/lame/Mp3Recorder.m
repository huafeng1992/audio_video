...
- (void)countVoiceTime
{
    ...
    if ([_delegate respondsToSelector:@selector(recordingMp3Recorder:recordTime:volume:)]) {
        [_delegate recordingMp3Recorder:self recordTime:recordTime volume:lowPassResults];
    }
}
...
//    [self deleteCafCache];
    NSLog(@"MP3转换结束");
    if (_delegate && [_delegate respondsToSelector:@selector(endConvertWithData:path:)]) {
        NSData *data = [NSData dataWithContentsOfFile:[self mp3Path]];
        [_delegate endConvertWithData:data path:[self mp3Path]];
    }
}

#pragma mark - Path Utils
- (NSString *)cafPath {
    NSString *cafPath = [NSTemporaryDirectory() stringByAppendingString:[NSString stringWithFormat:@"%@%@", self.fileName, @"tmp.caf"]];
    return cafPath;
}

- (NSString *)mp3Path {
    NSString *mp3Path = [NSTemporaryDirectory() stringByAppendingString:[NSString stringWithFormat:@"%@%@", self.fileName, @"mp3.caf"]];
    return mp3Path;
}
